...
export default class TabWorksLikeAsItShouldBe extends Plugin {
	...
	private handleTab(editor: Editor) {
	...
	private handleShiftTab(editor: Editor) {
	...
	private hasMultiLineSelection(editor: Editor): boolean {
	...
	private indentSelectedLines(editor: Editor) {
		const selections = editor.listSelections();
		const linesToIndent = new Set<number>();
	...
	private unindentSelectedLines(editor: Editor) {
		const selections = editor.listSelections();
		const linesToUnindent = new Set<number>();
		const linesThatWereUnindented = new Set<number>();
		...
		for (const line of sortedLines) {
			...
			if (lineText.startsWith("\t")) {
				editor.replaceRange(lineText.substring(1), 
					{ line, ch: 0 }, 
					{ line, ch: lineText.length }
				);
				...
				editor.replaceRange(lineText.substring(4), 
					{ line, ch: 0 }, 
					{ line, ch: lineText.length }
				);
				linesThatWereUnindented.add(line);
		...
		const newSelections = selections.map(selection => {
			const adjustChar = (line: number, ch: number): number => {
				if (!linesThatWereUnindented.has(line)) return ch;
	...
	private removeTabAtCursor(editor: Editor) {
		const cursor = editor.getCursor();
		const line = editor.getLine(cursor.line);
		...
		// Remove spaces at cursor position
			...
			if (spacesBefore > 0) {
				...
				editor.replaceRange(
					"",
					{ line: cursor.line, ch: spaceStart },
					{ line: cursor.line, ch: cursor.ch }
				);
			}
		}
